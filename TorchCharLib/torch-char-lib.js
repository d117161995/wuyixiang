const DB_URL = "./torch-char-db.json";
...
function renderSidebar() {
  ...
  function renderItem(c) {
    ...
    const fa = c.notes_episode ? `<span class="first-appear">${escapeHtml(c.notes_episode)}</span>` : "";
    const cn = c.notes_cn ? `<span class="cn-name">${escapeHtml(c.notes_cn)}</span>` : "";
    const label = (fa || cn) ? `${fa}${cn}` : escapeHtml(c.id);
...
function applyDb(raw) {
  ...
  const catWoman = state.characters.find(c => c.notes_cn === "猫女" || c.cn_name?.includes("猫女"));
  state.selected = catWoman?.id || state.characters[0]?.id || "";
  renderSidebar();
  renderMain();
}
...
async function loadDatabase() {
  ...
  if (window.__TORCH_CHAR_DB__ && Array.isArray(window.__TORCH_CHAR_DB__.characters)) {
    applyDb(window.__TORCH_CHAR_DB__);
    return;
  }
