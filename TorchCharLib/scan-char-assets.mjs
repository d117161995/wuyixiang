...
/**
 * Node.js 版本的角色资产扫描脚本
 * 等价于 scan-char-assets.py，扫描 X:\projects2024\TLI\Asset\Cha 下所有角色目录
 */
...
const CHA_ROOT = "X:\\projects2024\\TLI\\Asset\\Cha";
const OUTPUT_JSON = path.join(__dirname, "torch-char-db.json");
const OUTPUT_JS = path.join(__dirname, "torch-char-db.data.js");
...
function scanCharacter(charDir) {
  ...
  return {
    ...
    tex: { label: "贴图 Maya", path: texPath },
...
function main() {
  ...
  fs.writeFileSync(OUTPUT_JS, "window.__TORCH_CHAR_DB__ = " + JSON.stringify(db, null, 2) + ";\n", "utf-8");
